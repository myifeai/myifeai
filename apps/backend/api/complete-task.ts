...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { domain, xpPoints, taskText } = req.body; // Added taskText

    // 1. LOG THE TASK (Persistence)
    ...
    // 2. Update Profile XP
    await supabase.rpc('increment_xp', { user_id_input: userId, xp_to_add: xpPoints });

    // 3. Update Domain Score
    await supabase.rpc('increment_domain_score', { 
