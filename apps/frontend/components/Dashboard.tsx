...
export default function Dashboard({ profile }: { profile: ProfileData }) {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/complete-task`, {
        ...
        body: JSON.stringify({ 
          domain: task.domain, 
          xpPoints: task.xp,
          taskText: task.task // NEW: Passing text for the persistence log
        }),
      });
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#0b0612] gap-4">
      ...
      <p className="text-purple-400 font-bold tracking-widest animate-pulse uppercase">Syncing Neural Link...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0b0612] via-[#120a1f] to-[#050308] text-white px-6 py-10">
      <header className="max-w-6xl mx-auto mb-12 flex justify-between items-center">
        <div>
          ...
          <p className="text-white/40 text-[10px] uppercase tracking-[0.3em]">Operational Status: Optimal</p>
        </div>
      ...
      <section className="max-w-6xl mx-auto mb-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
             <GlassStat icon={<Zap className="text-yellow-400" />} label="Total XP" value={`${profile.xp_points}`} />
             <GlassStat icon={<TrendingUp className="text-blue-400" />} label="Current Rank" value={calculateRank(profile.xp_points)} />
          </div>
          <div className="bg-purple-500/10 border border-purple-500/20 rounded-2xl p-6 backdrop-blur-xl relative overflow-hidden">
            <MessageSquareQuote className="absolute -right-2 -bottom-2 text-white/5" size={100} />
            <p className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mb-3">CEO Briefing</p>
            <p className="text-sm leading-relaxed text-white/80 italic font-medium">"{plan.briefing || "Formulating today's strategy..."}"</p>
          </div>
        </div>
      </section>

      {/* INFINITE DOMAIN PROGRESS BARS */}
            ...
            <div key={s.domain} className="bg-white/5 border border-white/5 rounded-2xl p-4 hover:bg-white/10 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-white/60 uppercase">{s.domain}</span>
                  <span className="text-[8px] text-purple-400 font-bold uppercase tracking-tighter">LVL {Math.floor(s.score / 100) + 1}</span>
                </div>
                <span className="text-[10px] font-mono text-purple-400">{s.score % 100}%</span>
              </div>
              <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-gradient-to-r from-purple-600 to-blue-400 transition-all duration-1000"
        ...
        <div className="space-y-4">
          {plan.tasks.length > 0 ? plan.tasks.map((item, index) => (
            ...
            <div className="text-center p-16 rounded-3xl border border-dashed border-white/5 bg-white/[0.02]">
              ...
              <p className="text-white/30 text-xs font-medium tracking-widest uppercase italic">All Objectives Cleared</p>
            </div>
          )}
        </div>
      </section>
